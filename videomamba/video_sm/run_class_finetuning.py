...
def get_args():
    ...
    parser.add_argument('--sampling_rate', type=int, default=4)#skip
...
def custom_collate_fn(batch):
    ...
    videos, labels, paths = zip(*batch)  # Unzip batch into separate lists
    
    # Convert videos & labels to tensors, keep paths as list
    videos = torch.stack(videos)  # Adjust stacking based on tensor structure
    labels = torch.tensor(labels) 
    paths = paths  # Keep paths as a normal list (no unintended stacking)
    
    return videos, labels, paths
def main(args, ds_init):
    ...
    print("here", num_training_steps_per_epoch, args.epochs, args.weight_decay_end, args.weight_decay)
